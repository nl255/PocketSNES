		...
		for (i = 0; i < nblocks * 2; i++) {
			for (int j = i; j < nblocks * 2; j++) {
				if (blocks[j] == i) {
					...
					// memmove converted: Different addresses, or identical for blocks[i] ==
					// blocks[j] [Neb] DS2 DMA notes: Don't do DMA at all if blocks[i] == blocks[j]
...
bool8 CMemory::Init()
{
	...
	if (!RAM || !SRAM || !VRAM || !ROM || !BSRAM || !IPPU.TileCache[TILE_2BIT] || !IPPU.TileCache[TILE_4BIT] ||
...
again:
	...
	if ((ROM[0x7FD5] == 0x42) && (ROM[0x7FD6] == 0x13) &&
	    (strncmp("METAL COMBAT", (char *)&ROM[0x7FC0], 12) == 0)) {
		Settings.DisplayColor = BUILD_PIXEL(31, 0, 0);
		SET_UI_COLOR(255, 0, 0);
		S9xMessage(S9X_ERROR, S9X_ROM_CONFUSING_FORMAT_INFO, "Warning! Hacked Dump!");
	}
	...
	if (HeaderCount == 0 && !Settings.ForceNoHeader &&
		...
		__dcache_writeback_all();
		{
			...
			for (i = 0; i < TotalFileSize; i += 512) {
				ds2_DMAcopy_32Byte(2 /* channel: emu internal */, Memory.ROM + i, Memory.ROM + i + 512,
						   512);
				ds2_DMA_wait(2);
				ds2_DMA_stop(2);
			}
	...
	if (CalculatedSize > 0x400000 && !(ROM[0x7FD5] == 0x32 && ((ROM[0x7FD6] & 0xF0) == 0x40)) && // exclude S-DD1
...
uint32 CMemory::FileLoader(uint8 *buffer, const char *filename, int32 maxsize)
{
	...
	default:
		...
		do {
			...
			if ((FileSize - calc_size == 512 && !Settings.ForceNoHeader) || Settings.ForceHeader) {
				...
				__dcache_writeback_all();
				{
					...
					for (i = 0; i < calc_size; i += 512) {
						ds2_DMAcopy_32Byte(2 /* channel: emu internal */, ptr + i,
								   ptr + i + 512, 512);
						ds2_DMA_wait(2);
						ds2_DMA_stop(2);
					}
			...
			if ((ptr - ROM) < (maxsize + 0x200) && (isdigit(ext[0]) && ext[1] == 0 && ext[0] < '9')) {
				more = TRUE;
				ext[0]++;
#ifdef __WIN32__
				// memmove required: Overlapping addresses [Neb]
...
void S9xDeinterleaveMode2() { S9xDeinterleaveType2(); }
		...
		for (i = 0; i < nblocks * 2; i++) {
			for (int j = i; j < nblocks * 2; j++) {
				if (blocks[j] == i) {
					...
					// memmove converted: Different addresses, or identical if blocks[i] ==
					// blocks[j] [Neb]
					memmove(&Memory.ROM[blocks[j] * 0x10000], &Memory.ROM[blocks[i] * 0x10000],
						0x10000);
					// memmove converted: Different mallocs [Neb]
...
void CMemory::InitROM(bool8 Interleaved)
{
	...
	sprintf(
	    String, "\"%s\" [%s] %s, %s, Type: %s, Mode: %s, TV: %s, S-RAM: %s, ROMId: %s Company: %2.2s CRC32: %08X",
	    ROMName,
	    (ROMChecksum + ROMComplementChecksum != 0xffff || ROMChecksum != CalculatedChecksum) ? "bad checksum"
												 : "checksum ok",
...
#endif
#endif
	Settings.ForceHeader = Settings.ForceHiROM = Settings.ForceLoROM = Settings.ForceInterleaved =
	    Settings.ForceNoHeader = Settings.ForceNotInterleaved = Settings.ForceInterleaved2 = false;
}
...
void CMemory::BSLoROMMap()
{
	...
	/*  // Banks 40->7f and c0->ff
	  for (c = 0; c < 0x400; c += 16)
	  {
		      ...
		      for (i = c + 8; i < c + 16; i++)
			      Map [i + 0x400] = Map [i + 0xc00] = &ROM [((c << 11) + 0x200000) % CalculatedSize -
	  0x8000];
	...
	for (i = 0; i < 8; i++) {
		Map[0x205 + (i << 4)] = Map[0x285 + (i << 4)] = Map[0x305 + (i << 4)] = Map[0x385 + (i << 4)] =
		    Map[0x705 + (i << 4)];
		BlockIsRAM[0x205 + (i << 4)] = BlockIsRAM[0x285 + (i << 4)] = BlockIsRAM[0x305 + (i << 4)] =
		    BlockIsRAM[0x385 + (i << 4)] = TRUE;
		BlockIsROM[0x205 + (i << 4)] = BlockIsROM[0x285 + (i << 4)] = BlockIsROM[0x305 + (i << 4)] =
		    BlockIsROM[0x385 + (i << 4)] = FALSE;
	}
	for (c = 0; c < 8; c++) {
...
void CMemory::HiROMMap()
{
	...
	if (Settings.ForceSA1 ||
	    (!Settings.ForceNoSA1 && (ROMSpeed & ~0x10) == 0x23 && (ROMType & 0xf) > 3 && (ROMType & 0xf0) == 0x30)) {
		Settings.DisplayColor = BUILD_PIXEL(31, 0, 0);
		SET_UI_COLOR(255, 0, 0);
	}
...
void CMemory::TalesROMMap(bool8 Interleaved)
{
	...
	for (c = 0; c < 0x400; c += 16) {
		...
		if (c >= 0x300) {
			...
			BlockIsRAM[6 + c] = BlockIsRAM[7 + c] = BlockIsRAM[0x806 + c] = BlockIsRAM[0x807 + c] = TRUE;
		} else {
			Map[c + 6] = Map[c + 0x806] = (uint8 *)MAP_NONE;
			Map[c + 7] = Map[c + 0x807] = (uint8 *)MAP_NONE;
		}
...
void CMemory::SuperFXROMMap()
{
	...
	for (c = 0; c < 64; c++) {
#ifdef DS2_DMA
		ds2_DMAcopy_32Byte(2 /* channel: emu internal */, &ROM[0x200000 + c * 0x10000], &ROM[c * 0x8000],
				   0x8000);
		ds2_DMAcopy_32Byte(3 /* channel: emu internal 2 */, &ROM[0x208000 + c * 0x10000], &ROM[c * 0x8000],
				   0x8000);
		ds2_DMA_wait(2);
		ds2_DMA_wait(3);
		ds2_DMA_stop(2);
...
{
	static char tmp[30];
	static const char *CoPro[16] = {"DSP1",	    "SuperFX",	"OBC1",	    "SA-1",	   "S-DD1",    "S-RTC",
...
[14:25:27] <@Nach>     case 0x340f23e5: //Donkey Kong Country 3 (U) copier hack - handled
	*/
	...
	if (strcmp(ROMId, "AVCJ") == 0 ||
	    ...
	    // Soulblazer
	    strcmp(ROMName, "SOULBLAZER - 1 USA") == 0 || strcmp(ROMName, "SOULBLADER - 1") == 0 ||
	    ...
	    strcmp(ROMName, "HIOUDEN") == 0 || strcmp(ROMName, "ÃÝ¼É³À") == 0 || // Tenshi no Uta
	...
	if ((strcmp(ROMName, "SFX SUPERBUTOUDEN2") == 0) || (strcmp(ROMName, "ALIEN vs. PREDATOR") == 0) ||
	    (strcmp(ROMName, "STONE PROTECTORS") == 0) || (strcmp(ROMName, "SUPER BATTLETANK 2") == 0))
...
void CMemory::CheckForIPSPatch(const char *rom_filename, bool8 header, int32 &rom_size)
{
	...
	if (fread((unsigned char *)fname, 1, 5, patch_file) != 5 || strncmp(fname, "PATCH", 5) != 0) {
		fclose(patch_file);
		return;
	}
