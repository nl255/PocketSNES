...
static u32 sal_Input(int held)
{
	...
	if (keystate[SDLK_RCTRL] || (keystate[SDLK_ESCAPE] && keystate[SDLK_RETURN]))
		inputHeld |= SAL_INPUT_MENU;
	if (keystate[SDLK_ESCAPE] && keystate[SDLK_TAB])
		inputHeld |= SAL_INPUT_QUICKLOAD;
	if (keystate[SDLK_ESCAPE] && keystate[SDLK_BACKSPACE])
		inputHeld |= SAL_INPUT_QUICKSAVE;
...
const char *sal_DirectoryGetTemp(void) { return "/tmp"; }
...
u32 sal_VideoGetWidth() { return mScreen->w; }

u32 sal_VideoGetHeight() { return mScreen->h; }

u32 sal_VideoGetPitch() { return mScreen->pitch; }
...
void sal_VideoExitGame() { memset(mScreen->pixels, 0, SAL_SCREEN_WIDTH * SAL_SCREEN_HEIGHT * 2); }
...
void *sal_VideoGetBuffer() { return (void *)mScreen->pixels; }
...
void sal_VideoPaletteSet(u32 index, u32 color)
{
	*mPaletteCurr++ = index;
	*mPaletteCurr++ = color;
	if (mPaletteCurr > mPaletteEnd)
		mPaletteCurr = &mPaletteBuffer[0];
}
