...
local int unzlocal_GetCurrentFileInfoInternal(unzFile file, unz_file_info *pfile_info,
 ...
 */
local int unzlocal_CheckCurrentFileCoherencyHeader(unz_s *s, uInt *piSizeVar, uLong *poffset_local_extrafield,
						   uInt *psize_local_extrafield)
{
	...
	*psize_local_extrafield = 0;
	...
	if ((err == UNZ_OK) && s->cur_file_info.compression_method > Z_DEFLATED)
		err = UNZ_BADZIPFILE;
