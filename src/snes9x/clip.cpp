		...
		for (int w = 5; w >= 0; w--) {
			...
			//			if (!Settings.DisableGraphicWindows)
			{
				if (w == 5 || pClip->Count[5] ||
					...
					bool8 invert =
					    (w == 5 && ((c == 1 && (Memory.FillRAM[0x2130] & 0x30) == 0x10) ||
							...
							if (!PPU.ClipWindow1Inside[w]) {
								...
								if (PPU.Window1Left <= PPU.Window1Right) {
									if (PPU.Window1Left > 0) {
										...
										Win1[Window1Enabled++].Right =
										    PPU.Window1Left;
									}
									if (PPU.Window1Right < 255) {
										Win1[Window1Enabled].Left =
										    PPU.Window1Right + 1;
							...
							if (!PPU.ClipWindow2Inside[w]) {
								...
								if (PPU.Window2Left <= PPU.Window2Right) {
									if (PPU.Window2Left > 0) {
										...
										Win2[Window2Enabled++].Right =
										    PPU.Window2Left;
									}
									if (PPU.Window2Right < 255) {
										Win2[Window2Enabled].Left =
										    PPU.Window2Right + 1;
					...
					if (Window1Enabled && Window2Enabled) {
						...
						// 2. |    ----------------             | (Left >= 0, Right <= 255, Left
						// <= Right. One band)
						// 3. |------------           ----------| (Left1 == 0, Right1 < Left2;
						// Left2 > Right1, Right2 == 255. Two bands)
						...
						case CLIP_OR:
							if (Window1Enabled == 1) {
								if (BAND_EMPTY(Win1[0])) {
									B = Window2Enabled;
									// memmove converted: Different stack
									// allocations [Neb]
									memmove(Bands, Win2,
									...
									if (Window2Enabled == 1) {
										...
										if (BANDS_INTERSECT(Win1[0], Win2[0])) {
											...
											if (BANDS_INTERSECT(Bands[0],
												...
												B = 2;
										} else if (BANDS_INTERSECT(Win1[0],
													   Win2[1])) {
											Bands[B++] = Win2[0];
											OR_BANDS(Bands[B], Win1[0],
												 Win2[1]);
											B++;
										} else {
											Bands[0] = Win2[0];
								...
								if (BAND_EMPTY(Win2[0])) {
									...
									if (BANDS_INTERSECT(Win2[0], Win1[0])) {
										...
										if (BANDS_INTERSECT(Win2[0], Win1[1]))
											OR_BANDS(Bands[1], Win2[0],
												 Win1[1])
										else
											Bands[1] = Win1[1];
										B = 1;
										if (BANDS_INTERSECT(Bands[0], Bands[1]))
											OR_BANDS(Bands[0], Bands[0],
												 Bands[1])
										else
											B = 2;
						...
						case CLIP_XOR:
							if (Window1Enabled == 1 && BAND_EMPTY(Win1[0])) {
								...
								memmove(Bands, Win2, sizeof(Win2[0]) * Window2Enabled);
							} else if (Window2Enabled == 1 && BAND_EMPTY(Win2[0])) {
								B = Window1Enabled;
								// memmove converted: Different stack allocations [Neb]
								memmove(Bands, Win1, sizeof(Win1[0]) * Window1Enabled);
							} else {
								uint32 p = 0;
								uint32 points[10];
								uint32 i;
						...
						if (invert) {
							...
							if (j > 0) {
								if (j == 1) {
									...
									B = j;
									qsort((void *)Bands, B, sizeof(Bands[0]),
									      BandCompare);
