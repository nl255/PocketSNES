...
const char *S9xGetSnapshotDirectory(void) { return sal_DirectoryGetHome(); }
...
bool8_32 S9xDeinitUpdate(int Width, int Height, bool8_32)
{
	...
	case 0: /* No scaling */
		// case 3: /* Hardware scaling */
		{
			...
			u8 *src = (u8 *)IntermediateScreen,
			   *dst = (u8 *)sal_VideoGetBuffer() + ((sal_VideoGetWidth() - SNES_WIDTH) / 2) * sizeof(u16) +
	...
	case 1: /* Fast software scaling */
		if (PAL) {
			upscale_256x240_to_320x240((uint32_t *)sal_VideoGetBuffer(), (uint32_t *)IntermediateScreen,
						   SNES_WIDTH);
		} else {
			upscale_256x224_to_320x240((uint32_t *)sal_VideoGetBuffer(), (uint32_t *)IntermediateScreen,
						   SNES_WIDTH);
		}
		break;
	...
	case 2: /* Smooth software scaling */
		if (PAL) {
			upscale_256x240_to_320x240_bilinearish((uint32_t *)sal_VideoGetBuffer() + 160,
							       (uint32_t *)IntermediateScreen, SNES_WIDTH);
		} else {
			upscale_256x224_to_320x240_bilinearish((uint32_t *)sal_VideoGetBuffer() + 160,
							       (uint32_t *)IntermediateScreen, SNES_WIDTH);
		}
		break;
	}
...
uint32 S9xReadJoypad(int which1)
{
	...
	if (joy & SAL_INPUT_UP)
		val |= SNES_UP_MASK;
	if (joy & SAL_INPUT_DOWN)
		val |= SNES_DOWN_MASK;
	if (joy & SAL_INPUT_LEFT)
		val |= SNES_LEFT_MASK;
	if (joy & SAL_INPUT_RIGHT)
		val |= SNES_RIGHT_MASK;
	if (joy & SAL_INPUT_START)
		val |= SNES_START_MASK;
	if (joy & SAL_INPUT_SELECT)
...
void S9xSaveSRAM(int showWarning)
{
	if (CPU.SRAMModified) {
		...
		MenuMessageBox("SRAM saving ignored", "No changes have been made to SRAM", "",
			       MENU_MESSAGE_BOX_MODE_MSG);
	}
}
}

bool8_32 S9xOpenSoundDevice(int a, unsigned char b, int c) { return (TRUE); }
	...
	if (sound) {
		...
		*/
		...
		if (LastAudioRate != mMenuOptions.soundRate || LastStereo != mMenuOptions.stereo ||
			...
			sal_AudioInit(mMenuOptions.soundRate, 16, mMenuOptions.stereo, Memory.ROMFramesPerSecond);

			S9xInitSound(mMenuOptions.soundRate, mMenuOptions.stereo,
...
static inline int RunSound(void) { return Run(1); }

static inline int RunNoSound(void) { return Run(0); }
	...
	while (1) {
		...
		if (event == EVENT_LOAD_ROM) {
			...
			if (SnesRomLoad() == SAL_ERROR) {
				mRomName[0] = 0;
				MenuMessageBox("Failed to load ROM", mRomName, "Press any button to continue",
