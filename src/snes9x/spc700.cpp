...
#ifdef FAST_LSB_WORD_ACCESS
#define IndexedXIndirect() IAPU.Address = *(uint16 *)(IAPU.DirectPage + ((OP1 + IAPU.Registers.X) & 0xff));

#define Absolute() IAPU.Address = *(uint16 *)(IAPU.PC + 1);

#define AbsoluteX() IAPU.Address = *(uint16 *)(IAPU.PC + 1) + IAPU.Registers.X;

#define AbsoluteY() IAPU.Address = *(uint16 *)(IAPU.PC + 1) + IAPU.Registers.YA.B.Y;
...
#define IndirectIndexedY() IAPU.Address = *(uint16 *)(IAPU.DirectPage + OP1) + IAPU.Registers.YA.B.Y;
#else
...
#define AbsoluteX() IAPU.Address = OP1 + (OP2 << 8) + IAPU.Registers.X;

#define AbsoluteY() IAPU.Address = OP1 + (OP2 << 8) + IAPU.Registers.YA.B.Y;
...
void Apu9A()
{
	...
	if (((IAPU.Registers.YA.W ^ Work16) & 0x8000) && ((IAPU.Registers.YA.W ^ (uint16)Int32) & 0x8000))
		...
		APUClearOverflow();
	if (((IAPU.Registers.YA.W ^ Work16) & 0x0080) && ((IAPU.Registers.YA.W ^ (uint16)Int32) & 0x0080))
		APUSetHalfCarry();
	APUSetHalfCarry();
