...
static void C4DoScaleRotate(int row_padding)
{
	...
	//    READ_WORD(Memory.C4RAM+0x1f80)&0x1ff); printf("Matrix: [%10g %10g]  [%04x %04x]\n", A/4096.0, B/4096.0,
	//    A&0xffff, B&0xffff); printf("        [%10g %10g]  [%04x %04x]\n", C/4096.0, D/4096.0, C&0xffff, D&0xffff);
...
#endif
				int32 tan1 = (C4CosTable[angle1] != 0)
						 ? ((((int32)C4SinTable[angle1]) << 16) / C4CosTable[angle1])
						 : 0x80000000;
				int32 tan2 = (C4CosTable[angle2] != 0)
						 ? ((((int32)C4SinTable[angle2]) << 16) / C4CosTable[angle2])
						 : 0x80000000;
				int16 y = READ_WORD(Memory.C4RAM + 0x1f83) - READ_WORD(Memory.C4RAM + 0x1f89);
				int16 left, right;
				for (int j = 0; j < 225; j++) {
					if (y >= 0) {
						left = SAR((int32)tan1 * y, 16) - READ_WORD(Memory.C4RAM + 0x1f80) +
						       READ_WORD(Memory.C4RAM + 0x1f86);
						right = SAR((int32)tan2 * y, 16) - READ_WORD(Memory.C4RAM + 0x1f80) +
							READ_WORD(Memory.C4RAM + 0x1f86) +
