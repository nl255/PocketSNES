...
uint8 S9xSA1OPrint(char *Line, uint8 Bank, uint16 Address)
{
	...
	sprintf(Line, "%-44s A:%04X X:%04X Y:%04X D:%04X DB:%02X S:%04X P:%c%c%c%c%c%c%c%c%c HC:%03ld VC:%03ld", Line,
		SA1Registers.A.W, SA1Registers.X.W, SA1Registers.Y.W, SA1Registers.D.W, SA1Registers.DB,
...
/**********************************************************************************************/
void DPrint(char *Line) { printf("%s\n", Line); }
/**********************************************************************************************/
/* GetNumber()                                                                                */
...
void ProcessDebugCommand(char *Line)
{
	...
	if (*Line == 'S') {
		...
		printf("Sprites: Small: %dx%d, Large: %dx%d, OAMAddr: 0x%04x, OBJNameBase: 0x%04x, OBJNameSelect: "
		       "0x%04x, First: %d\n",
		       SmallWidth, SmallHeight, LargeWidth, LargeHeight, PPU.OAMAddr, PPU.OBJNameBase,
		...
		for (int i = 0; i < 3; i++)
			if (APU.TimerEnabled[i])
				printf("Timer%d enabled, Value: 0x%03X, 4-bit: 0x%02X, Target: 0x%03X\n", i,
				       APU.Timer[i], IAPU.RAM[0xfd + i], APU.TimerTarget[i]);
	}
	if (*Line == 'P') {
	...
	if (Line[0] == 'b') {
		...
		if (Line[1] == 'v') {
			Number = 0;
			if (GetNumber(Line + 2, &Number) == -1 && Number < 5) {
...
static void WhatsUsed()
{
	...
	if (PPU.BGMode == 7) {
		...
		printf("\nMatrix A: %.3f, B: %.3f, C: %.3f, D: %.3f, Centre X: %d Y:%d\n", (double)PPU.MatrixA / 256,
		       (double)PPU.MatrixB / 256, (double)PPU.MatrixC / 256, (double)PPU.MatrixD / 256, PPU.CentreX,
		       PPU.CentreY);
	}
	...
	for (i = 0; i < 8; i++) {
		if (missing.hdma_this_frame & (1 << i)) {
			printf("H-DMA %d [%d] 0x%02X%04X->0x21%02X %s %s 0x%02X%04X %s addressing\n", i,
			       DMA[i].TransferMode, DMA[i].ABank, DMA[i].AAddress, DMA[i].BAddress,
	...
	printf("VRAM write address: 0x%04x(%s), Full Graphic: %d, Address inc: %d\n", PPU.VMA.Address,
	       PPU.VMA.High ? "Byte" : "Word", PPU.VMA.FullGraphicCount, PPU.VMA.Increment);

	for (i = 0; i < 4; i++) {
		printf("BG%d: VOffset:%d, HOffset:%d, W:%d, H:%d, TS:%d, BA:0x%04x, TA:0x%04X\n", i, PPU.BG[i].VOffset,
		       PPU.BG[i].HOffset, (PPU.BG[i].SCSize & 1) * 32 + 32, (PPU.BG[i].SCSize & 2) * 16 + 32,
		       PPU.BG[i].BGSize * 8 + 8, PPU.BG[i].SCBase, PPU.BG[i].NameBase);
	}
	char *s = "";
	...
	printf("\nWindow 2 (%d, %d): ", PPU.Window2Left, PPU.Window2Right);
	for (i = 0; i < 6; i++)
