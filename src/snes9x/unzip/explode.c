...
int get_tree(l, n) unsigned *l; /* bit lengths */
...
int huft_free(t) struct huft *t; /* table to free */
...
void flush(w) unsigned w; /* number of bytes to flush */
...
void flush_stack(w) unsigned w; /* number of bytes to flush */
/* Do the equivalent of OUTB for the bytes slide[0..w-1]. */
{
