...
void S9xPrepareSoundForSnapshotSave (bool8 restore)
{
	...
	if (!restore)
	{
		for (i = 0; i < NUM_CHANNELS; i++)
		{
			...
			ch->count = 0;
			ch->envx = ch->xenvx >> 4;
			ch->envx_target = (short)(ch->xenvx_target >> 4);
			ch->direction = 0;
			ch-> left_vol_level = (short)((ch->xenvx * ch->volume_left ) >> 11);
			ch->right_vol_level = (short)((ch->xenvx * ch->volume_right) >> 11);
			ch->release_rate = 8;
...
void S9xFixSoundAfterSnapshotLoad (int version)
{
	...
	for (int i = 0; i < NUM_CHANNELS; i++)
	{
		...
		// TODO: just put them in savestate instead
...
void S9xResetSound (bool8 full)
{
	for (int i = 0; i < NUM_CHANNELS; i++)
	{
		...
		if(full)
		{
			...
			for(int j = 0 ; j < 4 ; j++) SoundData.channels[i].nb_sample[j] = 0;
			SoundData.channels[i].nb_index = 0;
		}
	}
