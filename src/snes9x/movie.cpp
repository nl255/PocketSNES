...
enum MovieState { MOVIE_STATE_NONE = 0, MOVIE_STATE_PLAY, MOVIE_STATE_RECORD };
...
int S9xMovieCreate(const char *filename, uint8 controllers_mask, uint8 opts, const wchar_t *metadata,
		   int metadata_length)
{
	return FILE_NOT_FOUND;
#if 0
...
bool8 S9xMovieActive() { return (Movie.State != MOVIE_STATE_NONE); }
...
bool8 S9xMovieUnfreeze(const uint8 *buf, uint32 size)
{
	...
	if (movie_id != Movie.MovieId || current_frame > max_frame || space_needed > size) {
		return false;
	}
	...
	if (!Movie.ReadOnly) {
		...
		flush_movie();
		fseek(Movie.File, Movie.ControllerDataOffset + (Movie.BytesPerFrame * (Movie.CurrentFrame + 1)),
		      SEEK_SET);
	} else {
