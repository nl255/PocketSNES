...
#define DSP4_READ_WORD(x) READ_WORD(DSP4.parameters + x)
...
void DSP4_Multiply(short Multiplicand, short Multiplier, int &Product) { Product = Multiplicand * Multiplier; }
...
void DSP4_Op01()
{
	...
	resume2:
		...
		if (project_focaly >= -0x0f)
			py_dy = (short)(project_focaly * project_focaly * -0.20533553 - 1.08330005 * project_focaly -
					69.61094639);
		else
		...
		printf("(line %d) Op01 check %02X, plane %04X, focal_y %04X, y2 %04X\n", c, (uint16)segments,
		       (uint16)(plane), (uint16)project_focaly, (uint16)project_y2);
#endif
	...
	sprite_found:
		...
		resume7:
			...
			printf("(line %d) %04X, %04X, %04X / %04X %04X\n", line, (uint16)sp_x, (uint16)sp_y,
			       (uint16)far_plane, (uint16)multi_farplane[multi_index1], (uint16)project_y2);
#endif
