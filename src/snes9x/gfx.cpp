...
#define FIX_INTERLACE(SCREEN, DO_DEPTH, DEPTH)                                                                         \
	if (IPPU.DoubleHeightPixels && ((PPU.BGMode != 5 && PPU.BGMode != 6) || !IPPU.Interlace))                      \
		for (uint32 y = GFX.StartY; y <= GFX.EndY; y++) {                                                      \
			...
			memmove(SCREEN + (y * 2 + 1) * GFX.Pitch2, SCREEN + y * 2 * GFX.Pitch2, GFX.Pitch2);           \
			if (DO_DEPTH) {                                                                                \
				/* memmove required: Same malloc, potentially overlapping addresses [Neb] */           \
				memmove(DEPTH + (y * 2 + 1) * (GFX.PPLx2 >> 1), DEPTH + y * GFX.PPL, GFX.PPLx2 >> 1);  \
			}                                                                                              \
		}
...
void DrawTileHalfWidth(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
void DrawClippedTileHalfWidth(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			      uint32 LineCount);
...
void DrawTilex2x2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
...
void DrawLargePixelHalfWidth(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Pixels, uint32 StartLine,
			     uint32 LineCount);
#endif
...
void DrawTile16HalfWidth(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
void DrawClippedTile16HalfWidth(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
				uint32 LineCount);
void DrawTile16x2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
void DrawClippedTile16x2(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			 uint32 LineCount);
void DrawTile16x2x2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
...
void DrawLargePixel16HalfWidth(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Pixels, uint32 StartLine,
			       uint32 LineCount);

void DrawTile16Add(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);

void DrawClippedTile16Add(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			  uint32 LineCount);

void DrawTile16Add1_2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);

void DrawClippedTile16Add1_2(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			     uint32 LineCount);

void DrawTile16FixedAdd1_2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);

void DrawClippedTile16FixedAdd1_2(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
				  uint32 LineCount);

void DrawTile16Sub(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);

void DrawClippedTile16Sub(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			  uint32 LineCount);

void DrawTile16Sub1_2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);

void DrawClippedTile16Sub1_2(uint32 Tile, uint32 Offset, uint32 StartPixel, uint32 Width, uint32 StartLine,
			     uint32 LineCount);

void DrawTile16FixedSub1_2(uint32 Tile, uint32 Offset, uint32 StartLine, uint32 LineCount);
...
void S9xSetupOBJ()
{
	...
	if (Settings.BGLayering)
		fprintf(stderr, "Entering SetupOBJS()\n");
#endif
...
#ifdef MK_DEBUG_RTO
	if (Settings.BGLayering)
		fprintf(stderr, "Priority rotation=%d, OAMAddr=%d -> ", PPU.OAMPriorityRotation,
			PPU.OAMAddr * 2 | (PPU.OAMFlip & 1));
#endif
	if (!PPU.OAMPriorityRotation || !(PPU.OAMFlip & PPU.OAMAddr & 1)) {
#ifdef MK_DEBUG_RTO
		if (Settings.BGLayering)
			fprintf(stderr, "normal FirstSprite = %02x\n", PPU.FirstSprite);
#endif
		/* normal case */
...
static void DrawOBJS(bool8 OnMain = FALSE, uint8 D = 0)
{
...
#ifdef MK_DEBUG_RTO
	if (Settings.BGLayering)
		fprintf(stderr, "Entering DrawOBJS() for %d-%d\n", GFX.StartY, GFX.EndY);
#endif
	CHECK_SOUND();
	...
	for (uint32 Y = GFX.StartY, Offset = Y * GFX.PPL; Y <= GFX.EndY; Y++, Offset += GFX.PPL) {
		...
		for (int S = GFX.OBJLines[Y].OBJ[I].Sprite; S >= 0 && I < 32; S = GFX.OBJLines[Y].OBJ[++I].Sprite) {
			...
			if (X == -256)
				X = 256;
			for (int t = tiles, O = Offset + X * GFX.PixSize;
				...
				if (X + 8 < NextPos) {
					...
					while (x < X + 8) {
						if (WinStat)
							(*DrawClippedTilePtr)(BaseTile | TileX, O, x - X, NextPos - x,
									      TileLine, 1);
						x = NextPos;
						for (; WinIdx < 7 && Windows[WinIdx].Pos <= x; WinIdx++)
							;
...
#ifdef MK_DEBUG_RTO
	if (Settings.BGLayering)
		fprintf(stderr, "Exiting DrawOBJS() for %d-%d\n", GFX.StartY, GFX.EndY);
#endif
}
		...
		for (uint32 clip = 0; clip < ClipCount; clip++) {
			...
			for (uint32 x = Left; x < Right; x += PixWidth,
				...
				if (BG.TileSize != 8) {
					...
					(*DrawLargePixelPtr)(Tile + (Quot & 1) * m5, s, HPos & 7, PixWidth, VirtAlign,
							     Lines);
			}
		}
	}
...
static void DrawBGMode7Background(uint8 *Screen, int bg) { RENDER_BACKGROUND_MODE7(uint8, (uint8)(b & GFX.Mode7Mask)) }
...
static void DrawBGMode7Background16Add(uint8 *Screen, int bg)
{
	RENDER_BACKGROUND_MODE7(uint16, *(d + GFX.DepthDelta)
					    ? (*(d + GFX.DepthDelta) != 1
						   ? COLOR_ADD(ScreenColors[b & GFX.Mode7Mask], p[GFX.Delta])
						   : COLOR_ADD(ScreenColors[b & GFX.Mode7Mask], GFX.FixedColour))
...
static void DrawBGMode7Background16Add1_2(uint8 *Screen, int bg)
{
	RENDER_BACKGROUND_MODE7(uint16, *(d + GFX.DepthDelta)
					    ? (*(d + GFX.DepthDelta) != 1
						   ? COLOR_ADD1_2(ScreenColors[b & GFX.Mode7Mask], p[GFX.Delta])
						   : COLOR_ADD(ScreenColors[b & GFX.Mode7Mask], GFX.FixedColour))
					    : ScreenColors[b & GFX.Mode7Mask]);
}
...
static void DrawBGMode7Background16Sub(uint8 *Screen, int bg)
{
	RENDER_BACKGROUND_MODE7(uint16, *(d + GFX.DepthDelta)
					    ? (*(d + GFX.DepthDelta) != 1
						   ? COLOR_SUB(ScreenColors[b & GFX.Mode7Mask], p[GFX.Delta])
						   : COLOR_SUB(ScreenColors[b & GFX.Mode7Mask], GFX.FixedColour))
					    : ScreenColors[b & GFX.Mode7Mask]);
}
...
static void DrawBGMode7Background16Sub1_2(uint8 *Screen, int bg)
{
	RENDER_BACKGROUND_MODE7(uint16, *(d + GFX.DepthDelta)
					    ? (*(d + GFX.DepthDelta) != 1
						   ? COLOR_SUB1_2(ScreenColors[b & GFX.Mode7Mask], p[GFX.Delta])
						   : COLOR_SUB(ScreenColors[b & GFX.Mode7Mask], GFX.FixedColour))
					    : ScreenColors[b & GFX.Mode7Mask]);
}
			...
			if (simpleCase) {                                                                              \
				...
				if ((aa < 460 && aa > -460) && (cc < 460 && cc > -460)) {                              \
					for (int x = startx; x != endx; x += dir, AA += aa, CC += cc, p++, d++) {      \
						...
						GFX.Z1 = Mode7Depths[(b & GFX.Mode7PriorityMask) >> 7];                \
						if (GFX.Z1 > *d && (b & GFX.Mode7Mask)) {                              \
							/* X10 and Y01 are the X and Y coordinates of the next source  \
							 * point over. */                                              \
                ...
                {                                                                                    \
					/* Find the dimensions of the square in the source image whose corners will be \
					 * examined. */                                                                \
					uint32 aaDelX = aa >> 1;                                                       \
					uint32 ccDelX = cc >> 1;                                                       \
					...
					for (int x = startx; x != endx; x += dir, AA += aa, CC += cc, p++, d++) {      \
						...
						GFX.Z1 = Mode7Depths[(b & GFX.Mode7PriorityMask) >> 7];                \
						if (GFX.Z1 > *d && (b & GFX.Mode7Mask)) {                              \
							/* X, Y, X10, Y10, etc. are the coordinates of the four pixels \
							 * within the */                                               \
...
void DisplayChar(uint8 *Screen, uint8 c)
{
	...
	if (Settings.SixteenBit) {
		...
		for (h = 0; h < font_height; h++, line++, s += GFX.PPL - font_width) {
...
static void S9xDisplayFrameRate()
{
	uint8 *Screen = GFX.Screen + 2 + (IPPU.RenderedScreenHeight - font_height - 1) * GFX.Pitch2;
	char string[10];
	int len = 5;
...
static void S9xDisplayString(const char *string)
{
	uint8 *Screen = GFX.Screen + 2 + (IPPU.RenderedScreenHeight - font_height * 5) * GFX.Pitch2;
...
#ifndef FOREVER_16_BIT
			Screen -= Settings.SixteenBit ? (font_width - 1) * sizeof(uint16) * max_chars
...
void S9xUpdateScreen()
{
	...
	GFX.Pseudo = (Memory.FillRAM[0x2133] & 8) != 0 && (GFX.r212c & 15) != (GFX.r212d & 15) && (GFX.r2131 == 0x3f);

#else

	GFX.Pseudo =
	    (Memory.FillRAM[0x2133] & 8) != 0 && (GFX.r212c & 15) != (GFX.r212d & 15) && (GFX.r2131 & 0x3f) == 0;
	...
	if (Settings.SupportHiRes &&
		...
		if (IPPU.Interlace && !IPPU.DoubleHeightPixels) {
			...
			for (register int32 y = (int32)GFX.StartY - 1; y >= 0; y--) {
				// memmove converted: Same malloc, different addresses, and identical addresses at line
				// 0 [Neb] DS2 DMA notes: This code path is unused [Neb]
				memmove(GFX.Screen + y * 2 * GFX.Pitch2, GFX.Screen + y * GFX.Pitch2, GFX.Pitch2);
				// memmove converted: Same malloc, different addresses [Neb]
				memmove(GFX.Screen + (y * 2 + 1) * GFX.Pitch2, GFX.Screen + y * GFX.Pitch2, GFX.Pitch2);
			}
		}
	} else if (!Settings.SupportHiRes) {
		if (PPU.BGMode == 5 || PPU.BGMode == 6 || IPPU.Interlace) {
