...
void S9xAPUSetEndX(int ch) { APU.DSP[APU_ENDX] |= 1 << ch; }
#ifdef __DJGPP
...
void S9xSetEnvRate(Channel *ch, unsigned long rate, int direction, int target)
{
	...
	static int64 steps[] = {//	0, 64, 1238, 1238, 256, 1, 64, 109, 64, 1238
				0,
	...
	if (rate == 0 || so.playback_rate == 0)
		...
		ch->erate = (unsigned long)(steps[ch->state] / (rate * so.playback_rate));
	}
}
...
void S9xSetFrequencyModulationEnable(uint8 byte) { SoundData.pitch_mod = byte & ~1; }
...
void S9xSetSoundADSR(int channel, int attack_rate, int decay_rate, int sustain_rate, int sustain_level,
		     int release_rate)
{
	Channel *ch = &SoundData.channels[channel];
	...
	case SOUND_DECAY:
		S9xSetEnvRate(ch, decay_rate, -1, (MAX_ENVELOPE_HEIGHT * (sustain_level + 1)) >> 3);
		break;
	case SOUND_SUSTAIN:
...
int S9xGetEnvelopeHeight(int channel)
{
	if ((Settings.SoundEnvelopeHeightReading || SNESGameFixes.SoundEnvelopeHeightReading2) &&
	    SoundData.channels[channel].state != SOUND_SILENT && SoundData.channels[channel].state != SOUND_GAIN) {
...
void S9xSetSoundSample(int channel, uint16 sample_number)
{
	...
	if (ch->state != SOUND_SILENT && sample_number != ch->sample_number) {
		int keep = ch->state;
		ch->state = SOUND_SILENT;
...
void AltDecodeBlock(Channel *ch)
{
	...
	if ((ch->last_block = filter & 1))
     ...
     (defined(__i386__) || defined(__i486__) || defined(__i586__) || defined(__WIN32__) || defined(__DJGPP)))
...
void DecodeBlock(Channel *ch)
{
	...
	if (Settings.EightBitConsoleSound) {
		...
		if (total_deviation_from_previous >= (int32)amplitude * 4) {
		...
		} else if (interim[0] < interim[1] && interim[1] < interim[2] && interim[2] < interim[3] &&
			   interim[4] > interim[5] && interim[5] > interim[6] && interim[6] > interim[7] &&
			   interim[7] > interim[8] && interim[8] > interim[9] && interim[9] > interim[10] &&
			   interim[10] > interim[11] && interim[12] < interim[13] && interim[13] < interim[14] &&
			...
			ch->decoded[12] = amplitude;
			   ...
			   interim[3] < interim[4] && interim[4] < interim[5] && interim[5] < interim[6] &&
			   interim[6] < interim[7] && interim[8] > interim[9] && interim[9] > interim[10] &&
			   interim[10] > interim[11] && interim[11] > interim[12] && interim[12] > interim[13] &&
			   interim[13] > interim[14] && interim[14] > interim[15]) {
	...
	for (uint32 J = 0; J < NUM_CHANNELS; J++) {
		...
		if (ch->needs_decode) {
			...
			if (Settings.InterpolatedSound && freq0 < FIXED_POINT && !mod)
				ch->interpolate = ((ch->next_sample - ch->sample) * (long)freq0) / (long)FIXED_POINT;
		}
			...
			if (ch->count >= FIXED_POINT) {
				...
				if (ch->type == SOUND_SAMPLE) {
					if (Settings.InterpolatedSound && freq < FIXED_POINT && !mod) {
						ch->interpolate =
						    ((ch->next_sample - ch->sample) * (long)freq) / (long)FIXED_POINT;
						ch->sample = (int16)(
						    ch->sample + (((ch->next_sample - ch->sample) * (long)(ch->count)) /
								  (long)FIXED_POINT));
					} else
						ch->interpolate = 0;
				} else {
	...
	if (so.sixteen_bit) {
				...
				if (so.stereo) {
					...
					if (FilterTapDefinitionBitfield == 0) {
						...
						for (J = 0; J < sample_count; J++) {
							...
							Echo[SoundData.echo_ptr] =
							    (E * SoundData.echo_feedback) / 128 + EchoBuffer[J];

							if ((SoundData.echo_ptr += 1) >= SoundData.echo_buffer_size)
								SoundData.echo_ptr = 0;

							I = (MixBuffer[J] * SoundData.master_volume[J & 1] +
							     E * SoundData.echo_volume[J & 1]) /
							    VOL_DIV16;
						...
						for (J = 0; J < sample_count; J++) {
							...
							Loop[(Z - 0) & 15] = E;
							E = E * FilterTaps[0];
							if (FilterTapDefinitionBitfield & 0x02)
								E += Loop[(Z - 2) & 15] * FilterTaps[1];
							if (FilterTapDefinitionBitfield & 0x04)
								E += Loop[(Z - 4) & 15] * FilterTaps[2];
							if (FilterTapDefinitionBitfield & 0x08)
								E += Loop[(Z - 6) & 15] * FilterTaps[3];
							if (FilterTapDefinitionBitfield & 0x10)
								E += Loop[(Z - 8) & 15] * FilterTaps[4];
							if (FilterTapDefinitionBitfield & 0x20)
								E += Loop[(Z - 10) & 15] * FilterTaps[5];
							if (FilterTapDefinitionBitfield & 0x40)
								E += Loop[(Z - 12) & 15] * FilterTaps[6];
							if (FilterTapDefinitionBitfield & 0x80)
...
#endif
		{
			if (SoundData.echo_enable && SoundData.echo_buffer_size) {
				...
				for (J = 0; J < sample_count; J++) {
					I = (MixBuffer[J] * SoundData.master_volume[J & 1]) / VOL_DIV8;
					CLIP8(I);
					buffer[J] = I + 128;
				}
			}
