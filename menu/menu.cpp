...
s32 LoadMenuOptions(const char *path, const char *filename, const char *ext, const char *optionsmem, s32 maxSize,
		    s32 showMessage)
{
...
s32 SaveMenuOptions(const char *path, const char *filename, const char *ext, const char *optionsmem, s32 maxSize,
		    s32 showMessage)
{
	s8 fullFilename[SAL_MAX_PATH];
	s8 _filename[SAL_MAX_PATH];
...
void PrintTitle(const char *title) { sal_ImageDraw(mMenuBackground, SAL_SCREEN_WIDTH, SAL_SCREEN_HEIGHT, 0, 0); }
...
int FileScan()
{
	...
	if (mRomList == NULL) {
		...
		if (mRomList == NULL) {
			// still no joy
			MenuMessageBox("Dude, I'm really broken now", "Restart system", "never do this again",
		...
		if (sal_DirectoryOpen(mRomDir, &d) == SAL_OK) {
			...
			while (sal_DirectoryRead(&d, &mRomList[x + startIndex], mRomDir) == SAL_OK) {
				...
				if (mRomList[x + startIndex].type == SAL_FILE_TYPE_FILE) {
					sal_DirectorySplitFilename(mRomList[x + startIndex].filename, path, filename,
								   ext);
					if (sal_StringCompare(ext, "zip") == 0 || sal_StringCompare(ext, "smc") == 0 ||
	...
	while (menuExit == 0) {
		...
		if (keys & INP_BUTTON_MENU_SELECT) {
			...
			case ROM_SELECTOR_SAVE_DEFAULT_DIR: // Save default directory
				...
				SaveMenuOptions(mSystemDir, DEFAULT_ROM_DIR_FILENAME, DEFAULT_ROM_DIR_EXT, mRomDir,
						strlen(mRomDir), 1);
				break;
			...
			default:
				...
				if (mRomList[focus].type == SAL_FILE_TYPE_DIRECTORY) {
					...
					if (sal_StringCompare(mRomList[focus].filename, ".") == 0) {
						...
						// Code below will never let you go further up than \SD Card\ on the
						// Gizmondo This is by design.
						sal_DirectoryGetParent(mRomDir);
						FileScan();
						focus = ROM_SELECTOR_DEFAULT_FOCUS; // default menu to non menu item
										    // just to stop directory scan being
										    // started
						smooth = focus << 8;
						sal_InputIgnore();
						break;
						...
						if (sal_StringCompare(mRomList[focus].filename, "/") == 0)
							...
							sal_DirectoryCombine(
							    mRomDir, mRomList[focus].filename); // go to sub directory
...
u32 IsPreviewingState() { return mPreviewingState; }
...
static s32 SaveStateSelect(s32 mode)
{
	...
	while (action != 0 && action != 100) {
		...
		if (keys & SAL_INPUT_UP || keys & SAL_INPUT_LEFT) {
			saveno--;
			action = 1;
		}
		if (keys & SAL_INPUT_DOWN || keys & SAL_INPUT_RIGHT) {
			saveno++;
			action = 1;
		}
		if (saveno < 0)
		...
		case 4:
			...
			if (mode == 0)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_DELETE_SAVESTATE) << 3)) >> 1, 190,
					       MENU_TEXT_DELETE_SAVESTATE, SAL_RGB(31, 31, 31));
			break;
		case 5: {
			u32 DestWidth = 205, DestHeight = 154;
			sal_VideoBitmapScale(0, 0, SNES_WIDTH, SNES_HEIGHT, DestWidth, DestHeight,
					     SAL_SCREEN_WIDTH - DestWidth, &mTempFb[0],
					     (u16 *)sal_VideoGetBuffer() +
						 (SAL_SCREEN_WIDTH * (((202 + 16) - DestHeight) / 2)) +
						 ((262 - DestWidth) / 2));
			sal_VideoDrawRect(0, 186, 262, 16, SAL_RGB(22, 0, 0));
			if (mode == 1)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_LOAD_SAVESTATE) << 3)) >> 1, 190,
					       MENU_TEXT_LOAD_SAVESTATE, SAL_RGB(31, 31, 31));
			else if (mode == 0)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_OVERWRITE_SAVESTATE) << 3)) >> 1, 190,
					       MENU_TEXT_OVERWRITE_SAVESTATE, SAL_RGB(31, 31, 31));
			else if (mode == 2)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_DELETE_SAVESTATE) << 3)) >> 1, 190,
					       MENU_TEXT_DELETE_SAVESTATE, SAL_RGB(31, 31, 31));
			break;
		...
		case 12:
			...
			sal_VideoPrint((262 - (strlen(MENU_TEXT_PREVIEW_SAVESTATE) << 3)) >> 1, 165,
				       MENU_TEXT_PREVIEW_SAVESTATE, SAL_RGB(31, 31, 31));
			if (mode == 1)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_LOAD_SAVESTATE) << 3)) >> 1, 175,
					       MENU_TEXT_LOAD_SAVESTATE, SAL_RGB(31, 31, 31));
			else if (mode == 0)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_OVERWRITE_SAVESTATE) << 3)) >> 1, 175,
					       MENU_TEXT_OVERWRITE_SAVESTATE, SAL_RGB(31, 31, 31));
			else if (mode == 2)
				sal_VideoPrint((262 - (strlen(MENU_TEXT_DELETE_SAVESTATE) << 3)) >> 1, 175,
					       MENU_TEXT_DELETE_SAVESTATE, SAL_RGB(31, 31, 31));
			break;
		case 13:
...
void ShowCredits()
{
	...
	while (!menuExit) {
		...
		// Draw screen:
		menuSmooth = menuSmooth * 7 + (menufocus << 8);
		menuSmooth >>= 3;
...
static void SettingsMenuUpdateText(s32 menu_index)
{
	...
	case SETTINGS_MENU_AUTO_SAVE_SRAM:
		sprintf(mMenuText[SETTINGS_MENU_AUTO_SAVE_SRAM], "Save SRAM on change         %s",
	...
	case SETTINGS_MENU_SOUND_ON:
		sprintf(mMenuText[SETTINGS_MENU_SOUND_ON], "Sound                       %s",
	...
	case SETTINGS_MENU_SOUND_STEREO:
		sprintf(mMenuText[SETTINGS_MENU_SOUND_STEREO], "Stereo                      %s",
	...
	case SETTINGS_MENU_FRAMESKIP:
		...
		case 0:
			strcpy(mMenuText[SETTINGS_MENU_FRAMESKIP], "Frameskip                  AUTO");
			break;
		default:
			sprintf(mMenuText[SETTINGS_MENU_FRAMESKIP], "Frameskip                     %1d",
...
{
	if (mRomName[0] != 0) {
		LoadMenuOptions(mSystemDir, mRomName, MENU_OPTIONS_EXT, (char *)mMenuOptions,
				sizeof(struct MENU_OPTIONS), 1);
	}
	return;
}
...
void MenuReloadOptions()
{
	if (mRomName[0] != 0) {
		...
		if (LoadMenuOptions(mSystemDir, mRomName, MENU_OPTIONS_EXT, (s8 *)mMenuOptions,
				    sizeof(struct MENU_OPTIONS), 0) == SAL_OK) {
			return;
		}
	}

	// Load global settings
	if (LoadMenuOptions(mSystemDir, MENU_OPTIONS_FILENAME, MENU_OPTIONS_EXT, (s8 *)mMenuOptions,
			    sizeof(struct MENU_OPTIONS), 0) == SAL_OK) {
		return;
	}
...
void MenuInit(const char *systemDir, struct MENU_OPTIONS *menuOptions)
{
	...
	if (LoadMenuOptions(mSystemDir, DEFAULT_ROM_DIR_FILENAME, DEFAULT_ROM_DIR_EXT, mRomDir, SAL_MAX_PATH, 0) !=
	    SAL_OK) {
...
static s32 SettingsMenu(void)
{
	...
	while (!menuExit) {
		...
		if (keys & INP_BUTTON_MENU_CANCEL) {
			...
			switch (menufocus) {
			case SETTINGS_MENU_LOAD_GLOBAL_SETTINGS:
				LoadMenuOptions(mSystemDir, MENU_OPTIONS_FILENAME, MENU_OPTIONS_EXT,
						(char *)mMenuOptions, sizeof(struct MENU_OPTIONS), 1);
				SettingsMenuUpdateTextAll();
				usleep(5e5);
				break;
			case SETTINGS_MENU_SAVE_GLOBAL_SETTINGS:
				SaveMenuOptions(mSystemDir, MENU_OPTIONS_FILENAME, MENU_OPTIONS_EXT,
						(char *)mMenuOptions, sizeof(struct MENU_OPTIONS), 1);
				usleep(5e5);
				break;
			case SETTINGS_MENU_LOAD_CURRENT_SETTINGS:
			...
			case SETTINGS_MENU_SAVE_CURRENT_SETTINGS:
				if (mRomName[0] != 0) {
					SaveMenuOptions(mSystemDir, mRomName, MENU_OPTIONS_EXT, (char *)mMenuOptions,
							sizeof(struct MENU_OPTIONS), 1);
					usleep(5e5);
				}
				break;
			...
			case SETTINGS_MENU_FRAMESKIP:
				if (keys & SAL_INPUT_RIGHT) {
					...
					if (mMenuOptions->frameSkip > 6)
						mMenuOptions->frameSkip = 0;
				} else {
					mMenuOptions->frameSkip--;
					if (mMenuOptions->frameSkip > 6)
						mMenuOptions->frameSkip = 6;
				}
				SettingsMenuUpdateText(SETTINGS_MENU_FRAMESKIP);
				break;
