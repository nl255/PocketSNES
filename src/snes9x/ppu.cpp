...
void S9xSetPPU(uint8 Byte, uint16 Address)
{
	...
	if (Address <= 0x2183) {
		...
		case 0x210D:
			...
			//        PPU.BG[0].HOffset, CPU.V_Counter);
			break;
		...
		case 0x210E:
			...
			//        PPU.BG[0].VOffset, CPU.V_Counter);
			break;
		...
		case 0x210F:
			...
			//        PPU.BG[1].HOffset, CPU.V_Counter);
			break;
		...
		case 0x2110:
			...
			//        PPU.BG[1].VOffset, CPU.V_Counter);
			break;
		...
		case 0x2111:
			...
			//        PPU.BG[2].HOffset, CPU.V_Counter);
			break;
		...
		case 0x2112:
			...
			//        PPU.BG[2].VOffset, CPU.V_Counter);
			break;
		...
		case 0x2113:
			...
			//        PPU.BG[3].HOffset, CPU.V_Counter);
			break;
		...
		case 0x2114:
			...
			//        PPU.BG[3].VOffset, CPU.V_Counter);
			break;
		...
		case 0x2139:
			...
			if (!PPU.VMA.High) {
				if (PPU.VMA.FullGraphicCount) {
					...
					IPPU.VRAMReadBuffer =
					    READ_WORD(Memory.VRAM + ((PPU.VMA.Address << 1) & 0xffff));
...
void S9xSetCPU(uint8 byte, uint16 Address)
{
	...
	if (Address < 0x4200) {
		...
		case 0x4200:
			...
			if (byte & 0x20) {
				// if(!SNESGameFixes.umiharakawaseFix && PPU.IRQVBeamPos==262) fprintf(stderr,
				// "PPU.IRQVBeamPos = %d, CPU.V_Counter = %d\n", PPU.IRQVBeamPos, CPU.V_Counter);
...
void S9xUpdateJustifiers()
{
	...
	if (Memory.FillRAM[0x4201] & 0x80) {
		...
		if (!offscreen) {
			...
			if ((!last_p1 && IPPU.Controller == SNES_JUSTIFIER) ||
			    (last_p1 && IPPU.Controller == SNES_JUSTIFIER_2)) {
				PPU.VBeamPosLatched = (uint16)(y + 1);
				PPU.HBeamPosLatched = (uint16)x;
				PPU.HVBeamCounterLatched = TRUE;
				Memory.FillRAM[0x213F] |= 0x40 | Model->_5C78;
...
void REGISTER_2118(uint8 Byte)
{
	...
	if (!PPU.VMA.High) {
		...
		if (Settings.TraceVRAM && !CPU.InDMA) {
			printf("VRAM write byte: $%04X (%d,%d)\n", PPU.VMA.Address, Memory.FillRAM[0x2115] & 3,
