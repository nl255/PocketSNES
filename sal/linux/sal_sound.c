...
static void sdl_audio_callback(void *userdata, Uint8 *stream, int len)
{
	...
	if (Muted) {
		...
		memmove(stream, &Buffer[LocalReadPos * BytesPerSample],
			(BUFFER_SAMPLES - LocalReadPos) * BytesPerSample);
		memmove(&stream[(BUFFER_SAMPLES - LocalReadPos) * BytesPerSample], &Buffer[0],
			(SamplesRequested - (BUFFER_SAMPLES - LocalReadPos)) * BytesPerSample);
	} else {
...
void sal_AudioPause(void) { SDL_PauseAudio(1); }

void sal_AudioResume(void) { SDL_PauseAudio(0); }

void sal_AudioClose(void) { SDL_CloseAudio(); }

void sal_AudioGenerate(u32 samples)
{
	u32 SamplesAvailable, LocalReadPos = ReadPos /* isolate a bit against races with the audio thread */,
...
{
	u32 SamplesBuffered, LocalReadPos = ReadPos /* isolate a bit against races with the audio thread */,
			     LocalWritePos = WritePos /* keep a non-volatile copy at hand */;
	if (LocalReadPos <= LocalWritePos)
		SamplesBuffered = LocalWritePos - LocalReadPos;
	else
...
u32 sal_AudioGetMinFrames() { return BUFFER_FRAMES - 1; }

u32 sal_AudioGetMaxFrames() { return BUFFER_FRAMES; }

u32 sal_AudioGetSamplesPerFrame() { return SamplesPerFrame; }

u32 sal_AudioGetBytesPerSample() { return BytesPerSample; }

void sal_AudioSetVolume(s32 l, s32 r) {}

void sal_AudioSetMuted(u32 muted) { Muted = muted; }
