...
static char *S9xMnemonics[256] = {"NOP",
				  "TCALL0",
				  "SET0 $%02X",
				  "BBS0 $%02X,$%04X",
				  "OR A,$%02X",
...
int S9xAPUOPrint(char *buffer, uint16 Address)
{
	...
	sprintf(buffer, "%s %-20s A:%02X X:%02X Y:%02X S:%02X P:%c%c%c%c%c%c%c%c %03dl %04dl %04dl", buffer, mnem,
